...
import (
	...
	"github.com/TE0dollary/chatlog-bot/internal/wechatdb/repository"
)
...
func (w *DB) Initialize() error {
	ds, err := datasource.New(w.path, w.platform, w.version)
	if err != nil {
		return err
	}
	w.repo, err = repository.New(ds)
	if err != nil {
		return err
	}
	return nil
...
func (w *DB) SetCallback(group string, callback func(event fsnotify.Event) error) error {
	return w.repo.SetCallback(group, callback)
}
