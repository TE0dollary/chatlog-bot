...
// ValidateDerivedKey 验证已派生的 encKey（跳过昂贵的第一阶段 PBKDF2，只需 2 轮派生 macKey）
func ValidateDerivedKey(page1, encKey, salt []byte, hashFunc func() hash.Hash, hmacSize, reserve, pageSize int) bool {
	if len(encKey) != KeySize {
		return false
	}

	macSalt := XorBytes(salt, 0x3a)
	macKey := pbkdf2.Key(encKey, macSalt, 2, KeySize, hashFunc)
	...
	return hmac.Equal(mac.Sum(nil), page1[dataEnd:dataEnd+hmacSize])
}
