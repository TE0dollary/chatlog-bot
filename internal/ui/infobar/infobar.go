...
const (
	...
	// column indices
	labelCol1 = 0 // label column 1
	valueCol1 = 1 // value column 1
	labelCol2 = 2 // label column 2
	valueCol2 = 3 // value column 2
	totalCols = 4
)
...
func New() *InfoBar {
	...
	table.SetCell(
		autoDecryptRow,
		labelCol2,
		tview.NewTableCell(fmt.Sprintf(" [%s::]%s", headerColor, "Derived Keys:")),
	)
...
// UpdateDerivedKeyCount updates the Derived Keys count display.
func (info *InfoBar) UpdateDerivedKeyCount(count int) {
	if count == 0 {
		info.table.GetCell(autoDecryptRow, valueCol2).SetText("[not fetched]")
	} else {
		info.table.GetCell(autoDecryptRow, valueCol2).SetText(fmt.Sprintf("%d keys", count))
	}
}
