...
func New(path, name, envPrefix string, writeConfig bool) (*Manager, error) {
	v := viper.NewWithOptions(viper.KeyDelimiter("::"))
	v.SetConfigType(DefaultConfigType)
	var err error

	// Path: 默认使用程序运行目录下的 ./data
	if len(path) == 0 {
		path, err = os.Getwd()
		if err != nil {
			path = "."
		}
		path += string(os.PathSeparator) + "data"
	}
	...
	// Name: 默认配置文件名为 config（生成 config.yaml）
	if len(name) == 0 {
		name = "config"
	}
...
// Save encodes v (a struct) into viper's internal state via mapstructure tags,
// then writes the config file once. Fields tagged with `mapstructure:"-"` are skipped.
func (c *Manager) Save(v interface{}) error {
